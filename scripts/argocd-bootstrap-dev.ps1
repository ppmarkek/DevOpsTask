...
& (Join-Path $RootDir "scripts\bootstrap-secrets.ps1") -SecretName "wp-dev-db-credentials" -Context $Context
...
if ($secret) {
    ...
    Write-Host ""
    Write-Host " GitOps: push to 'develop' -> CI pushes GHCR -> Image Updater syncs wp-dev"
    Write-Host "============================================"
}
